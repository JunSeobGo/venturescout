...
def test_invoke_repairs_malformed_json_once(monkeypatch):
    responses = iter(
        [
            '{"summary": "broken" "risks": []}',
            '{"summary": "broken", "risks": []}',
        ]
    )
    requested_model_ids = []
    ...
    monkeypatch.setitem(sys.modules, "boto3", fake_boto3)
    monkeypatch.setenv("AGENT_LLM_PROVIDER", "bedrock")
    monkeypatch.setenv("BEDROCK_SONNET_MODEL_ID", "sonnet-repair")
    ...
    assert requested_model_ids == ["sonnet-repair", "sonnet-repair"]
    assert output["summary"] == "broken"
    assert output["risks"] == []
    assert output["llm_succeeded"] is True
