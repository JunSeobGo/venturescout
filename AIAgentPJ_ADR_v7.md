...
### ADR-046 — (기각) NLI 모델로 stance 산출
- **상태**: rejected (측정 후 배선 철회)
- **맥락**: ADR-045에서 stance 산출 코드가 아예 없다는 게 드러났다. NLI(자연어 추론)는 (전제, 가설)을 entailment/contradiction/neutral로 가르므로 stance 정의와 1:1로 대응하고, 로컬 CPU 모델이라 **비용 0**이다. 유력한 후보로 보였다.
- **시도**: `cross-encoder/nli-deberta-v3-base`를 붙여 `retrieve()`의 rerank 직전에 태깅. 사람 라벨 21건(관련있음 문서의 stance)과 대조.
- **측정 결과 — 두 축 모두 실패**:
    ```
    일치율        7/21 = 33.3%
    예측 분포     neutral 20 / contradicts 1 / supports 0
    혼동행렬      supports → neutral 13 (전부 놓침)
                  neutral  → contradicts 1 (오탐)
    검색 지연     문서당 3,288ms → retrieve 1회당 +33초 (p50 95ms 대비 350배)
    ```
    33%는 "전부 neutral로 찍기"와 같은 수준이다. 예측의 95%가 neutral이었으니 **모델이 사실상 아무 판단도 하지 못했다.**
- **원인 분석**:
    1. **도메인 불일치** — MNLI 학습 데이터는 일상 문장이다. 제품 설명·특허 청구항은 분포 밖.
    2. **과제 정의가 다르다** — NLI의 entailment는 *논리적 함의*("문서가 참이면 가설도 반드시 참")인데, 여기 필요한 건 *증거적 지지*("가설을 뒷받침하는 정황인가")다. "Deel은 글로벌 급여 플랫폼이다"는 "글로벌 급여 플랫폼들이 규정준수로 경쟁한다"를 논리적으로 함의하지 않지만 증거로는 충분하다. **사람이 supports로 본 걸 모델이 neutral로 본 것은 NLI 기준으로는 오히려 맞다.** 과제를 잘못 매핑한 것이다.
    3. 낮은 확신을 neutral로 떨어뜨리는 임계값(0.5)이 보수적으로 작동해 쏠림을 키웠다.
- **결과**: 배선을 철회하고 검색 지연을 복구했다(p50 정상 범위). `pipeline/stance.py`는 남기되 **배선되지 않았음과 기각 사유를 헤더에 명시**했다 — guardrails.py처럼 죽은 코드가 조용히 남는 일을 반복하지 않기 위해서다. 임계값→라벨 변환 로직과 테스트 10건은 LLM 백엔드에서 재사용한다.
- **다음 안 (LLM 배치 태깅)**: 문서당 1콜이 아니라 **10건을 1콜로 묶는 것**이 핵심이다. NLI가 느렸던 건 하나씩 처리했기 때문이다.
    | 방식 | 비용 | 지연 | 정확도 |
    |---|---|---|---|
    | A 검색 시점 (10건/콜) | +$0.034/분석 (기존 대비 +11%) | +2~4s | 높음(실제 가설 사용) |
    | **B 적재 시점 사전 태깅** | **$0.61 1회** | **0** | 근사(축 대표문장) |
    B를 먼저 한다 — rerank는 절대값이 아니라 상대 순위만 쓰므로 축 단위 근사로도 반박 근거를 상위로 올리는 효과는 난다. 정확도가 더 필요하다고 확인되면 그때 A로 올린다.
- **교훈**: "비용 0"이 채택 근거가 될 수 없다. 정확도와 지연을 재기 전까지는 후보일 뿐이고, 이번엔 둘 다 실패했다.
...
### ADR-048 — (발견) 검색 지표의 천장 + stance 축별 변별력
- **상태**: accepted
- **맥락**: "검색 성능이 왜 안 오르는가"를 추적하다 두 가지를 찾았다. 둘 다 **측정 쪽 문제**였고, 성능 최적화를 시도하기 전에 먼저 고쳐야 할 것이었다.

#### 1. precision@5의 천장이 0.457이었다
    query_id    정답  P@5천장  실측
    saas-h1       1     0.2    0.0
    saas-h2       0     0.0    0.0   ← 무엇을 검색해도 0점
    saas-h3       9     1.0    1.0   만점
    fintech-h1    1     0.2    0.2   만점
    hr-h2         6     1.0    0.8
    patent-h4     2     0.4    0.4   만점
    patent-h5     2     0.4    0.4   만점
    평균                0.457  0.400  (달성률 87.5%)

P@5는 분모가 5인데 쿼리당 정답이 1~2건이라 완벽히 검색해도 0.2~0.4가 최대다. 7개 중 4개가 이미 천장이었고, 개선 여지는 12.5%뿐이었다. **ADR-045에서 rerank 가중치 14조합이 전부 같은 점수를 낸 것도 같은 이유다** — 움직일 여지가 있는 쿼리가 2개뿐이었다. 가중치가 무의미한 게 아니라 측정이 그걸 담을 수 없었다.

원인은 라벨셋 구성이다. 후보를 `retrieve()` 한 번의 상위 10건에서만 뽑았고, 그중 relevant가 1~2건이면 분모를 채울 정답이 애초에 존재하지 않는다. 더 나쁜 건 **하이브리드가 놓친 문서는 영영 후보에 못 들어와 "정답이 아닌 것"으로 취급**된다는 점이다 — recall이 구조적으로 과대평가된다.

- **조치**:
    - **pooling**(TREC 방식): 벡터 전용 / 키워드 전용 / 하이브리드 세 설정의 상위 15건을 합집합으로 모은다. 실측으로 `bm-usage`는 벡터가 14건, `ip-cart`는 키워드가 13건을 더 건졌다 — **하이브리드 단독 풀이 실제의 절반 수준이었다.**
    - 쿼리 7 → 25개(5축 × 5). 쿼리가 적으면 하나가 지표의 14%를 좌우해 노이즈가 크다.
    - 정답 0건인 쿼리를 평균에서 제외하고 `unscorable_queries`로 드러낸다. 포함하면 **올바른 동작(관련 문서가 없으니 못 찾음)을 0점으로 처벌**한다.
    - `precision_at_k_ceiling`을 함께 낸다. 실측만 보면 0.4가 실패로 읽히지만 천장이 0.457이면 만점에 가깝다.
    - 라벨링은 haiku 초벌 → 사람 확인 방식으로 바꿨다(554건 / 70콜 / $0.10). 초벌은 `_llm_*`에 따로 쓴다 — 지표가 읽는 칸으로 바로 승격하면 모델을 모델로 채점하는 자기참조가 된다.
- **잠정 결과**(초벌을 정답으로 가정, 사람 확인 전):
    ```
    P@5 천장  0.457 → 0.96     달성률  87.5% → 66.7%
    NDCG      0.619 → 0.798    recall  0.704 → 0.316
    contradiction_coverage  None → 0.119   (처음으로 계산됨)
    ```
    **recall이 떨어진 것이 정직해진 것이다.** 후보 풀이 넓어져 실제로 놓치는 양이 드러났다.

#### 2. stance는 축마다 변별력이 다르다 — 코퍼스 편향 탓
소량(20쌍, $0.01)으로 먼저 재고 전체를 돌리는 규율을 세웠다. 결과가 갈렸다.

    대상                              판정 분포                         결론
    seed_review × customer_problem    supports 전부                     기각
    patent × technology               supports 10/10                    기각
    patent × ip                       neutral 10/10                     기각
    seed_review × business_model      contradicts 68 / neutral 22       채택
    seed_competitor × competition     sup 36 / neu 47 / con 7           채택
    seed_competitor × business_model  sup 36 / neu 53 / con 1           채택
    seed_pricing × business_model     sup 27 / neu 42 / con 21          채택

- **기각 사유 — 셋 다 원인이 다르다**:
    1. `seed_review × customer_problem`: 리뷰 90건이 **전부 `sentiment: negative`**다. "고객이 문제를 겪는다"는 가설에 모든 부정 리뷰가 supports다.
    2. `patent × technology`: 축 문장이 "현재 기술로 구현 가능하다"인데 **특허는 정의상 구현된 기술을 기술한다.** 특허의 존재 자체가 supports다.
    3. `patent × ip`: "이 기법이 기존 청구항과 겹쳐 침해 위험이 있다"는 **질의(사용자 아이디어)와 특허의 관계**에 달려 있다. 축 대표 문장만으로는 "무엇과 겹치는가"를 알 수 없다. **ADR-046에서 "축 대표문장으로 근사한다"던 방식 B의 근본 한계**가 여기서 드러났다 — ip 축만은 검색 시점 태깅(방식 A)이 아니면 안 된다.
- **인용 충실도**: 리뷰 67/68 · 경쟁사 7/8 · 가격 14/16. 특허에서는 환각 인용이 1건 나왔다(`Auction System and Method`). 도메인이 멀수록 떨어진다.
- **비용**: 검증 $0.03 → 본 실행 $0.13. 검증 없이 전체를 돌렸으면 **변별력 0인 특허 440쌍에 $0.22를 버렸다.**
- **교훈**: 코퍼스 편향은 지표를 조용히 무력화한다. "모든 문서가 같은 판정"은 모델이 틀린 게 아니라 **축 문장과 코퍼스가 애초에 한 방향**이라는 뜻이다. 전체를 돌리기 전 소량으로 판정 분포부터 보는 것이 싸다.

---

### ADR-049 — (수정) 하이브리드 가중치의 스케일 불일치 — minmax 정규화 채택
- **상태**: accepted (잠정 라벨 기준, 사람 확인 후 재검증 필요)
- **맥락**: ADR-045에서 "hybrid 0.6/0.4가 측정 없이 정해졌다"를 열어뒀고, rerank 가중치 14조합이 전부 같은 점수를 낸 것도 미해결로 남아 있었다. ADR-048에서 지표 천장을 걷어낸 뒤 다시 보니, 가중치 자체보다 **두 항의 스케일이 다르다**는 게 근본 문제였다.
- **측정 — 명목 가중치가 실제 가중치가 아니다**: 상위 20건에서 두 항의 값 분포를 쟀다. 순위를 가르는 것은 절대값이 아니라 **폭**이다.
    ```
    source_type        cos 중앙   cos 폭   ts 폭   keyword 실제 기여율
    seed_pricing        0.5175   0.1925  0.8359        74.3%
    seed_review         0.3975   0.1848  0.1391        33.4%
    seed_competitor     0.4973   0.1706  0.7303        74.1%
    patent              0.4814   0.1632  0.8087        76.8%
                                                  (명목 40%)
    ```
    `1 - cosine`은 0.46~0.65 좁은 구간에 몰려 폭이 0.17인데, `ts_rank`는 0~0.96으로 폭이 0.73~0.84다. 0.4를 곱해도 키워드가 이긴다. **명목 40%인 키워드가 실제로는 순위 변별의 74~77%를 좌우했고, seed_review만 33%였다** — 전역 상수 하나가 코퍼스마다 다르게 동작하고 있었다.
- **조치**: `search_documents`의 후보 생성 CTE에 `ROW_NUMBER()`를 추가해 두 항의 **원점수와 순위를 모두** 내려보내고, 합성식을 `config.fusion_mode`로 갈아끼울 수 있게 했다.
    | 방식 | 설명 |
    |---|---|
    | weighted | 기존. 원점수에 가중치를 곱해 더한다 |
    | **minmax** | 후보군 안에서 각 항을 0~1로 편 뒤 가중합 — **채택** |
    | rrf | 원점수를 버리고 순위만 쓴다(1/(k+rank)) |
- **측정 결과**(잠정 라벨셋 616건 / 25쿼리, LLM 초벌을 정답으로 가정):
    ```
    방식                  P@5   recall    MRR   NDCG  contra
    weighted 0.6/0.4     0.632  0.313  0.750  0.799  0.119   ← 기존
    weighted 0.5/0.5     0.616  0.305  0.718  0.772  0.119
    weighted 0.8/0.2     0.624  0.304  0.750  0.793  0.167
    minmax   0.6/0.4     0.648  0.318  0.749  0.808  0.214   ← 채택
    rrf                  0.544  0.265  0.767  0.792  0.262
    ```
    **가중치가 비로소 작동한다.** ADR-045에서 14조합이 전부 같은 점수를 냈던 것과 대조적으로, 정규화 후 minmax 스윕은 실제 곡선을 그린다:
    ```
    vector  0.3    0.4    0.5    0.6    0.7    0.8    0.9
    P@5    0.568  0.600  0.632  0.648  0.632  0.584  0.592
    NDCG   0.728  0.744  0.768  0.808  0.800  0.801  0.809
    MRR    0.715  0.715  0.743  0.749  0.768  0.777  0.785
    ```
    P@5·recall은 0.6에서 최고, MRR은 0.9까지 단조 증가한다. **원래의 0.6/0.4 자체는 좋은 선택이었고, 정규화가 없어 그 의도대로 동작하지 않았을 뿐이다.** 가중치는 유지하고 정규화만 넣는다.
- **RRF를 채택하지 않은 이유**: MRR(0.767)과 contradiction_coverage(0.262)는 가장 높지만 P@5가 0.648 → 0.544로 크게 떨어진다. 에이전트가 프롬프트에 넣는 근거는 top-5이므로 그 구간의 정밀도가 우선이다. 다만 "첫 정답을 위로 올리는" 용도나 반박 근거 노출에는 RRF가 낫다 — `FUSION_MODE=rrf`로 남겨 뒀다. RRF는 순위만 쓰므로 vector/keyword 가중치를 바꿔도 결과가 **완전히 동일**했다(설계상 당연하며, 구현이 의도대로임을 보여준다).
- **한계**: 잠정 라벨(LLM 초벌) 기준이라 절대값을 믿으면 안 된다. 다만 **스케일 불일치는 라벨과 무관한 실측 사실**이고 정규화는 그 자체로 정당하다. 사람 확인 후 `eval/regression.py`로 재검증한다.
- **곁가지 — 회귀 실행기 추가**: 지표를 잴 때마다 스크립트를 새로 써서 돌린 탓에 "NDCG 0.598 → 0.619"가 코드 변경 때문인지 라벨 수정 때문인지 헷갈렸고 실제로 한 번 틀렸다. `eval/regression.py`가 테스트 + 지표를 한 번에 돌리고 기준선과 diff를 찍는다. 설정(fusion_mode·가중치·임베딩 모델)도 함께 저장해 **변화의 원인을 구분**할 수 있게 했다.

---

### ADR-050 — (기각) 임베딩 모델 교체 — PatentSBERTa는 제 역할을 하고 있다
- **상태**: rejected (측정 후 기각)
- **맥락**: `AI-Growth-Lab/PatentSBERTa`를 코퍼스 전체에 쓰고 있다. 490건 중 270건이 특허가 아닌 제품 리뷰·경쟁사·가격 문서라, **특허용 파인튜닝 모델을 비특허 텍스트에 쓰는 도메인 불일치**가 검색 품질의 병목이라고 의심했다. 비특허 270건만 범용 모델로 재임베딩하면 75초라 시도 비용도 낮아 보였다.
- **측정**: 25개 쿼리에 대해 정답 문서와 오답 문서의 cosine 유사도 평균을 비교했다(라벨은 사람 확인분 + LLM 초벌). 모델이 "관련 있는 것"과 "없는 것"을 실제로 갈라놓는지가 교체 여부의 선행 질문이다.
    ```
    축                  정답 평균  오답 평균     차이  쿼리
    business_model       0.4913   0.4502  +0.0411     5
    competition          0.4491   0.3780  +0.0710     5
    customer_problem     0.4147   0.3802  +0.0344     5
    ip                   0.4678   0.4131  +0.0547     5
    technology           0.4543   0.4009  +0.0534     5

    전체                  0.4661   0.3982  +0.0678    25
    효과크기 Cohen's d = 0.748   (표본 정답 291 / 오답 325)
    ```
- **결과 — 가설이 틀렸다**:
    1. **모든 축에서 정답 > 오답**이고 d=0.748은 중간~큰 효과다. 모델이 변별을 하고 있다.
    2. **비특허 축이 더 나쁘지 않다.** 오히려 `competition`(경쟁사 문서, 비특허)이 가장 잘 갈리고(+0.0710) 특허 축도 정상 작동한다(ip +0.0547, technology +0.0534). 도메인 불일치 가설을 뒷받침하는 신호가 없다.
    3. 가장 안 갈리는 축은 `customer_problem`(+0.0344)인데, **그 축은 코퍼스가 전부 `sentiment: negative`라 stance도 무력했던 축이다**(ADR-048). 모델 문제가 아니라 코퍼스 편향으로 일관되게 설명된다.
- **왜 처음에 의심했나 — 오독**: 상위 20건의 cosine 폭이 0.16~0.19로 좁은 것을 변별력 부족으로 읽었다(ADR-049의 스케일 측정). 그러나 **상위 20건은 정의상 서로 가까운 문서들**이라 폭이 좁은 게 정상이다. 변별력을 보려면 상위권 내부가 아니라 **정답 대 오답**을 갈라 봐야 한다. 같은 데이터에서 잘못된 결론을 낼 뻔했다.
- **결정**: 모델 교체를 하지 않는다. 재임베딩 비용(시간)과 스키마 변경(실험용 embedding 컬럼 추가)을 감수할 근거가 없다. 검색 품질을 올릴 여지는 임베딩이 아니라 **코퍼스 편향**(customer_problem 축)과 **후보 생성**(ADR-048 pooling) 쪽에 있다.
- **교훈**: "도메인이 안 맞을 것 같다"는 그럴듯한 가설이었고 시도 비용도 낮았지만, **재보니 틀렸다.** 낮은 비용은 착수 근거가 되지 못한다 — ADR-046에서 "비용 0이 채택 근거가 될 수 없다"고 적은 것의 대칭형이다.
